...
/*While::While(Expression *e, Statement *s) : 
    ...
    stmt(s) 
{
    // Cria um novo rótulo para o início do laço
    start = NewLabel();
    end = NewLabel();  // Define um rótulo para o final do laço
}*/

void While::Gen()
{
    cout << "L" << start << ":" << endl;  // Etiqueta de início do loop

    Expression *cond = Rvalue(expr);  // Avalia a condição como valor-r

    // Verifica a condição; se falso, salta para o fim
    cout << "\tifFalse " << cond->ToString() << " goto L" << end << endl;

    stmt->Gen();  // Gera o código para o corpo do loop

    cout << "\tgoto L" << start << endl;  // Volta ao início para reavaliar a condição

    cout << "L" << end << ":" << endl;  // Etiqueta de fim do loop
}
