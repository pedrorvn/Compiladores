...
Logical::Logical(Token *t, Expression *e1, Expression *e2) : Expression(NodeType::LOG, ExprType::BOOL, t), expr1(e1), expr2(e2)
{
    ...
    if (expr1->type != ExprType::BOOL || expr2->type != ExprType::BOOL)
    {
        if (expr1->type == ExprType::INT || expr2->type == ExprType::INT)
        {
            ...
            {
                int int_value2 = std::stoi((expr2->Name()));
                bool bool_value2 = (int_value2 != 0);
                Token *bool_token2 = new Token(*expr2->token);
                bool_token2->lexeme = bool_value2 ? "true" : "false";
                expr2 = new Expression(NodeType::LOG, ExprType::BOOL, bool_token2);
            }
        }
        else
        {
            stringstream ss;
            ss << "\'" << token->lexeme << "\' usado com operandos não booleanos/inteiros ("
...
If::If(Expression *e, Statement *s) : Statement(NodeType::IF_STMT), expr(e), stmt(s) {
    if (expr->type != ExprType::BOOL) {
        // Se for inteiro, converte para booleano (onde 0 é falso e qualquer outro valor é verdadeiro)
        if (expr->type == ExprType::INT) {
            expr->type = ExprType::BOOL; // Aqui você altera o tipo para booleano
        } else {
            // Caso contrário, lança um erro
            stringstream ss;
            ss << "A expressão do 'if' precisa ser booleano ou inteiro (encontrado: "
               << expr->Type() << ")";
            throw SyntaxError{scanner->Lineno(), ss.str()};
        }
    }
...
While::While(Expression *e, Statement *s) : Statement(NodeType::WHILE_STMT), expr(e), stmt(s) {
    if (expr->type != ExprType::BOOL) {
        if (expr->type == ExprType::INT) {
            ...
            stringstream ss;
            ss << "A expressão do 'while' precisa ser booleano ou inteiro (encontrado: "
               << expr->Type() << ")";
            throw SyntaxError{scanner->Lineno(), ss.str()};
        }
    }
...
DoWhile::DoWhile(Statement *s, Expression *e) : Statement(NodeType::DOWHILE_STMT), stmt(s), expr(e) {
    if (expr->type != ExprType::BOOL) {
        if (expr->type == ExprType::INT) {
            ...
            stringstream ss;
            ss << "A expressão do 'dowhile' precisa ser booleano ou inteiro (encontrado: "
               << expr->Type() << ")";
            throw SyntaxError{scanner->Lineno(), ss.str()};
        }
    }
